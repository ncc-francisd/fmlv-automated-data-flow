...
## Width is the mirrors-folded figure, on the Admiral as well as the coachbuilts

Marquis print `OVERALL WIDTH (MIRRORS FOLDED)` for every layout. On the K-Yachts, the KEAs
and the KEA Kompakt that is the habitation body, which overhangs the mirrors. **On the
Admiral K 6.3 it is the mirrors**: a Ducato's body is about 2050 mm, which is what FMLV
holds, and its folded mirrors reach 2260 mm.

It is recorded as published on both, per the requester's decision of 15 September 2026 —
one column has to mean one thing, and that thing is what the manufacturer states with the
mirrors folded. See `README.md`. **This proposes `ADMIRAL K 6.3: 2050 → 2260`**, which is
what run #86 proposed before a shorter-lived rule blanked it.
